#!/usr/bin/env bash
# Stand up swerve's Jenkins job(s) — job-as-code, mirroring lyku/jenkins/setup-jenkins.sh.
#
# Env: JENKINS_URL (default http://localhost:8080), JENKINS_USER (default muzzin),
#      JENKINS_TOKEN (a Jenkins API token — required).
# Needs: java + curl. The job builds https://github.com/airgap/swerve.git (branch dev)
...
echo "  java -jar $CLI -s $JENKINS_URL -auth $JENKINS_USER:\$JENKINS_TOKEN build swerve-ci"
