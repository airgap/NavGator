//! swerve-engine — the single crate that depends on the Servo fork.
//!
//! Everything the rest of swerve uses from the engine is re-exported here, so the
...
//! types. The next step is to replace these re-exports with a servo-free swerve API
//! surface (defined alongside `swerve-protocol`), so a Servo type change touches only
//! this crate, not the app.

pub use servo::{
    DevicePoint, InputEvent, Key, KeyState, KeyboardEvent, MouseButton, MouseButtonAction,
