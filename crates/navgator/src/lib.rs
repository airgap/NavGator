...
/// Run the app on a prepared event loop (shared by the desktop and Android entry points).
fn run(event_loop: EventLoop<WakeUp>) {
...
/// Desktop entry point (invoked by the thin `src/main.rs` binary).
pub fn desktop_main() {
...
/// Android entry point — invoked by android-activity's NativeActivity glue.
