...
navgator builds on **[`airgap/swervo`](https://github.com/airgap/swervo)** — our
...
| Fork | Upstream | navgator pins | Consumed via |
| --- | --- | --- | --- |
...
in navgator's `Cargo.toml` redirects all 8 stylo crates to our fork at the same rev. `webrender` is
...
| Branch / remote | Role |
| --- | --- |
| `airgap/swervo` `main` | Our **integration line**. Starts identical to upstream `ed1af70`; our patches land here. navgator's `Cargo.toml` pins a commit on this branch. |
...
navgator consumes the fork through `Cargo.toml`:
...
the merge (clones into `$NAVGATOR_FORKS_DIR`, fetches upstream, merges, pushes). Per repo it does:
...
# then bump navgator's Cargo.toml rev to the new main commit
```
...
| --- | --- | --- | --- |
| `patches/navgator-ua` | swervo | `315497b` | Brands the default User-Agent — appends a `NavGator/0.1.0` token to all 6 per-OS UA strings in `components/config/prefs.rs` (keeps the Firefox/Servo compat tokens). The **first fork patch**: proves the patch → pin → build → ship pipeline end-to-end. navgator-engine pins this commit. |
...
The `navgator-ci` Pipeline job is **job-as-code** (`jenkins/job-configs/navgator-ci.xml`,
created/updated via `jenkins/setup-jenkins.sh`): it builds `dev` from
`github.com/airgap/NavGator.git` via the root `Jenkinsfile` and polls every 5 min (a
...
`navgator-ci-base` image as lyku does with `lyku-ci-base`.
