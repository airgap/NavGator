# swerve — Security, Sandboxing & Site Isolation
...
`/home/nicole/.cargo/git/checkouts/servo-e53a6e7b994a25fe/ed1af70` and the swerve repo.

> **One-line verdict:** swerve today is a **single-process, unsandboxed** browser whose
...
| External control socket | `SWERVE_IPC` Unix socket, **no auth, default umask perms**, full navigation/tab control. Off unless env var set. | `src/main.rs:763-772`. |
| TLS / network | Inherited from Servo `net` (rustls + aws-lc-rs). No swerve-side policy. | below. |
...
2. **`SWERVE_IPC` is an unauthenticated local control plane.** Any local process /
...
embedder-controllable. swerve must (a) never expose an "ignore all cert errors" toggle
...
| **G9** | **Unauthenticated `SWERVE_IPC` + permissive socket perms.** | **Medium (local)** | Local privilege/automation surface; must be hardened or release-gated off. |
...
```
              ┌──────────────────────────────────────────────────────────┐
 PRIVILEGED   │  swerve browser process (broker)                          │
...
`spawn_multiprocess` path can be swerve-controlled rather than gaol-hardcoded — but note
...
- Move the chrome **off `file://`** onto an internal **`swerve://` privileged scheme**
...
- The chrome webview runs in the **broker** but with a hard rule: **it must never load
  ...
  unique). Treat any chrome navigation to non-`swerve://` as a bug → block (swerve
  already denies `swerve:` command nav; extend to a strict allowlist).
...
- Define a **permissions model** (geolocation, camera/mic, notifications, clipboard) —
  Servo's `embedder` permission hooks exist; swerve must implement deny-by-default
  prompts.

### 3.5 Safe-Browsing-equivalent, privacy-preserving (fixes G6)

Do **not** call Google Safe Browsing (telemetry/dependency on the ecosystem swerve
...
- **Fuzzing**: stand up `cargo-fuzz`/libFuzzer + OSS-Fuzz-style continuous fuzzing of
  the parsers swerve is exposed through (URL, HTML/CSS via Servo, image/font decoders).
...
- **JIT hardening**: ensure SpiderMonkey ships with W^X / its JIT mitigations on; don't
  disable.
- Treat every `unsafe` block in swerve's own code as reviewed; minimize FFI surface.
...
1. **Sandboxed multiprocess content, on by default, on every shipping platform**
   ...
   → resolves **G1, G4**.
2. **Privileged chrome on an internal `swerve://` scheme with strict CSP and a typed,
...
6. **`SWERVE_IPC` off by default + `0600` + per-user runtime dir + token auth** (or no
...
| --- | --- | --- | --- |
| 1 | Turn on Servo multiprocess+sandbox in swerve; implement the embedder `--content-process` branch and `ServoBuilder.opts(multiprocess+sandbox)`; verify per-domain processes spawn. | Servo API (`run_content_process`) | weeks (engine API is ready) |
...
| 4 | `swerve://` privileged chrome scheme + CSP + typed bridge. | — (parallel) | weeks |
...
`constellation`/`servo` crates, not the embedder.** swerve's security therefore inherits
...
- swerve single-process / file:// chrome / IPC: `/raid/swerve/src/main.rs:79-83, 104-119, 251, 326-347, 568, 763-772`.
