# swerve — software architecture to scale the prototype
...
> All "current state" facts below are read from `/raid/swerve/src/`,
...
1. **Decompose the single binary into a Cargo workspace of ~10 crates _now_,
   while it is cheap.** One `swerve-servo` crate is the *only* crate that may
   `use servo::*` — it is the firebreak against Servo churn (the thing that
   killed Verso). Everything else talks to it through a swerve-owned, stable
...
3. **Build the chrome as a real app with a tiny reactive layer, not a framework.**
   ...
   Define a versioned, typed `swerve:`-message protocol to replace the current
   `location.href = "swerve:..."` hack.
...
| Fact | Value | Source |
| --- | --- | --- |
| Crates | **1** binary crate (`swerve`) | `Cargo.toml` |
...
| Engine↔UI bridge | In-process. Chrome→engine via `location.href = "swerve:..."` intercepted in `request_navigation`; engine→chrome via `WebView::evaluate_javascript` dispatching `swerve:state` CustomEvent | `main.rs`, `chrome.js` |
| External control | Opt-in Unix-socket text protocol (`SWERVE_IPC`), ~7 verbs in / 2 events out | `main.rs` `start_ipc` |
...
- **`ProtocolHandler`/`ProtocolRegistry`** (`protocol_handler` module) → register
  a real `swerve://` (or `about:`) scheme for internal pages (settings, history,
  newtab) instead of `file://` + a fake `swerve:` command scheme.
...
in a **separate process** (versoview) behind an `ipc-channel` protocol. swerve
...
```
   ...
   │                                           └───────────┬────────────┘      │
   │                                                       ▼                    │
   │                                        swerve-engine (owns Servo,          │
   │                                        tabs, OffscreenRenderingContext,    │
   │                                        WebViewDelegate impl)               │
   └───────────────────────────────────────────────────────────────────────────┘

   v1 transport: in-process function calls / mpsc.   Later: ipc-channel + a
   versioned protocol crate, with swerve-engine as a child process.
```

Concretely: define two swerve-owned enums, `EngineCommand` (Navigate, NewTab,
...
`swerve:`-URL bridge, and the `SWERVE_IPC` socket) all collapse into "produce an
`EngineCommand`; consume an `EngineEvent`." The existing `SWERVE_IPC` text
...
├── crates/
│   ├── swerve-servo/              # ★ THE ONLY crate that `use servo::*`
...
│   ├── swerve-protocol/           # EngineCommand/EngineEvent enums + (de)serialization.
│   │   │                          #   NO servo dep. Versioned. Shared by engine + chrome bridge
│   │   │                          #   + IPC + future out-of-process client.
│   ├── swerve-app/                # binary: winit event loop, window, wires engine↔chrome↔services.
│   ├── swerve-chrome-bridge/      # Rust side of the chrome protocol (parse swerve: msgs,
│   │   │                          #   serialize state pushes); host of internal swerve:// pages.
│   ├── swerve-data/               # ★ profiles + "places" SQLite DB (history/bookmarks/autofill/
│   │   │                          #   permissions/site-settings) + blob store. rusqlite (bundled).
│   ├── swerve-settings/           # typed settings schema, layered (default→profile→sync), watch.
│   ├── swerve-theme/              # theme model + compiler → CSS vars + userstyles
│   │   │                          #   (drives UserContentManager.add_stylesheet).
│   ├── swerve-sync/               # "Lyku" client: account, E2E-encrypted records, conflict
│   │   │                          #   resolution; pluggable transport (self-host later).
│   ├── swerve-updater/            # update check + verify (signature) + stage/apply.
│   ├── swerve-ipc/                # external control transport (Unix socket / named pipe) over
│   │   │                          #   swerve-protocol. (Today's start_ipc, promoted.)
│   └── swerve-testkit/            # WebDriver client, Xvfb harness wrappers, golden-image helpers.
├── chrome/                        # the chrome app source (HTML/CSS/TS) — see §5
│   └── (built assets embedded via rust-embed or a build.rs into swerve-chrome-bridge)
...
```
swerve-app ──► swerve-servo ──► [servo, embedder_traits]   ← ONLY here
   │  │  │  └─► swerve-data, swerve-settings, swerve-theme, swerve-sync,
   │  │  │        swerve-updater, swerve-ipc, swerve-chrome-bridge
   │  │  └────► swerve-protocol  (also used by swerve-servo, swerve-ipc, chrome-bridge)
   └─ every non-engine crate depends on swerve-protocol, NEVER on swerve-servo's servo types.
```

| Crate | Depends on `servo`? | Why it exists / scaling payoff |
| --- | --- | --- |
| `swerve-servo` | **Yes (only one)** | Quarantines churn. A Servo bump touches *this crate and its tests*, not the app. ~`main.rs` engine+tab+delegate logic moves here. |
| `swerve-protocol` | No | Stable, versioned vocabulary. Enables in-proc → out-of-proc swap and the external-engine (Tauri) goal without re-plumbing. |
| `swerve-app` | transitively | Thin: window, event loop, service wiring. The "main" today minus engine internals. |
| `swerve-chrome-bridge` | No | Replaces the `swerve:` `location.href` hack with a typed channel; serves internal pages. |
| `swerve-data` | No | History/bookmarks/autofill = embedder's job (Servo doesn't do it). Independent, unit-testable without Servo. |
| `swerve-settings` | No | Layered config; feeds theme + engine prefs. Sync-aware. |
| `swerve-theme` | No | Theme→CSS compiler. Output consumed by chrome (CSS vars) + `UserContentManager` (page userstyles). |
| `swerve-sync` | No | Lyku. Can be developed/tested entirely headless. |
| `swerve-updater` | No | Platform-specific apply, signature verify. |
| `swerve-ipc` | No | External control plane (M5). Already prototyped. |
| `swerve-testkit` | dev-only | WebDriver + Xvfb + golden images. |
...
```
$XDG_DATA_HOME/swerve/profiles/<id>/
├── places.sqlite          # history, bookmarks, autofill, permissions, tab-session
├── places.sqlite-wal      # (WAL mode)
├── settings.json          # swerve-settings (layered over compiled defaults)
...
`swerve-data` exposes async, Servo-free APIs: `record_visit`, `query_history`,
...
and theme compiler. Keep swerve settings (UI/theme/sync) distinct from Servo
...
| Styling | **Plain CSS with custom properties** (`--swerve-*`) as the theming substrate; no CSS-in-JS, no Tailwind build | themes = swapping CSS-var values; cheap, Servo-friendly |
| Routing | hash/path within the chrome doc for settings/history/newtab "pages" served via `swerve://` protocol handler | avoids `file://` and the fake-scheme hack |
...
### 5.2 Replace the `swerve:` `location.href` bridge with a typed protocol

Today the chrome does `window.location.href = "swerve:nav#" + url`, intercepted
...
- **Chrome → engine**: a small injected JS shim `window.swerve.send(cmd)` that
  serializes a `swerve-protocol` `EngineCommand` to JSON and ships it over **one**
  channel. Cleanest mechanism at this rev: register a **`swerve://command`
  ...
  (`ProtocolHandler` is the more durable choice and also gives you `swerve://`
  internal pages.)
- **Engine → chrome**: keep `WebView::evaluate_javascript` dispatching a single
  `swerve:event` `CustomEvent` carrying a JSON `EngineEvent`. Typed on both
  sides via shared TS types generated from `swerve-protocol` (e.g. `ts-rs` or a
...
(autocomplete from `swerve-data`, search suggestions, security indicator),
...
GX-class), newtab page, error pages, `about:` pages (`swerve://memory`,
`swerve://settings`, etc.). All of this is `EngineEvent`-driven DOM — which is
...
1. **Chrome theming** — `swerve-theme` compiles a theme (colors, radii, density,
   accent, optional background image/video, animation toggles) into a set of
   `--swerve-*` CSS custom properties injected into the chrome document. Switching
...
`swerve-protocol`/`swerve-theme` so chrome, engine, and sync agree.
...
The chrome bundle is embedded into `swerve-chrome-bridge` via `rust-embed`/`build.rs`
...
| Platform | Engine (Servo) | swerve gaps to close | Priority |
| --- | --- | --- | --- |
...
Keep platform-specific code in clearly-scoped modules (`swerve-ipc` transport,
`swerve-updater` apply, `swerve-app` window/menu). winit + Servo already abstract
...
| --- | --- | --- | --- |
| **Unit** | `cargo test` per crate | `swerve-data` (SQLite queries, frecency), `swerve-settings` (layering), `swerve-theme` (compile), `swerve-protocol` (round-trip ser/de), omnibox parsing | every commit, fast (no Servo) |
...
| **Engine integration** | `swerve-servo` tests behind a feature, headless | boot Servo, load a page, assert events fire (load/title/url) | PR (slow, GPU/Xvfb) |
| **UI / e2e** | **WebDriver** via `Servo::execute_webdriver_command` + `webdriver_server` (servoshell pattern) + `swerve-testkit` client | type in omnibox→navigate; open/close tabs; settings toggles persist; theme switch | PR / nightly |
...
| **WPT** | Servo's WPT runner (`tests/wpt` exists in tree) against swerve's engine config | regression-track web-platform conformance for our pinned rev + our prefs | nightly |
...
Promote the `xdotool` smoke run into a checked-in script under `swerve-testkit`
...
1. **Servo churn (existential — killed Verso).** Mitigation: single
   `swerve-servo` quarantine crate, servo-free `swerve-protocol`, pinned rev,
...
- Is the **`ProtocolHandler`** path mature enough at this rev to host
  `swerve://settings` etc. and to carry chrome→engine commands, or do we keep the
...
- **Lyku** protocol shape: CRDT vs last-writer-wins per record? E2E key
  ...
  gates `swerve-sync`'s data model — keep records in `swerve-protocol`.)
...
| --- | --- | --- |
| **A0** | Workspace split: extract `swerve-servo`, `swerve-protocol`, `swerve-app` from `main.rs`; promote `IpcCommand`→`EngineCommand`. No behavior change. | the firewall; parallel work |
| **A1** | `swerve-data` (SQLite places DB) + `swerve-settings` + per-profile `config_dir`. | history/bookmarks/omnibox/settings |
| **A2** | Typed chrome protocol (`swerve://` handler or structured fragment + JSON); TS build via esbuild; signals store. | a real chrome app |
...
| **A4** | `swerve-theme` + chrome customization studio; `UserContentManager` userstyles/blocking. | the differentiator |
| **A5** | `swerve-testkit` (WebDriver UI suite + Xvfb smoke + WPT) wired into CI; `xtask bump-servo` gate. | sustainable churn-handling |
| **A6** | `swerve-updater` + signed channels; macOS/Windows ports (named-pipe IPC, packaging). | shippable cross-platform |
| **A7** | `swerve-sync` (Lyku) over `swerve-protocol`; self-host server later. | settings/data sync |
...
- Local ground truth: cached Servo tree at
  ...
  `components/config/opts.rs`, `ports/servoshell/*`), and swerve repo
